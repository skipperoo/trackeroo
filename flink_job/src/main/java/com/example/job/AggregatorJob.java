...
public class AggregatorJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String kafkaUrl = System.getenv().getOrDefault(
            "KAFKA_URL",
            "kafka:9092"
        );
        String postgresUrl = System.getenv().getOrDefault(
            "POSTGRES_URL",
            "jdbc:postgresql://tsdb:5432/tracker_db?sslmode=disable"
        );

        System.out.printf(
            ">>> [DEBUG] Kafka: %s | Postgres: %s%n",
            kafkaUrl,
            postgresUrl
        );
        ...
        // Parse JSON messages
        DataStream<Envelope> parsed = rawStream
            .map(value -> {
                System.out.printf(
                    ">>> [KAFKA_MSG] Received at %s: %s%n",
                    Instant.now(),
                    value
                );
                try {
                    Envelope envObj = handler
                        .getMapper()
                        .readValue(value, Envelope.class);
                    System.out.printf(
                        ">>> [PARSED_OK] dev_id=%s tag=%s%n",
                        envObj.dev_id,
                        envObj.tag
                    );
                    return envObj;
                    ...
                    System.err.printf(
                        ">>> [JSON_PARSE_ERROR] %s - Message: %s%n",
                        e.getMessage(),
                        value
                    );
        ...
        DataStream<Envelope> validParsed = parsed
            .filter(e -> e.dev_id != null && !e.dev_id.equals("unknown"))
            .setParallelism(2);
                ...
                JdbcSink.sink(
                    ...
                    (ps, record) -> {
                        try {
                            ...
                            System.out.printf(
                                ">>> [DB_INSERT/UPSERT] dev_id=%s route=%s ok%n",
                                record.dev_id,
                                record.route_hash
                            );
                        } catch (Exception e) {
                            System.err.printf(
                                ">>> [DB_ERROR] dev_id=%s route=%s | %s%n",
                                record.dev_id,
                                record.route_hash,
                                e.getMessage()
                            );
